...
The data warehouse is engineered using a **Star Schema** centered on `Fact_Transactions` to ensure fast query aggregation, low latency, and efficient relational filtering.
                      ...
                      +-----------+------------+
                                  |
+----------------------+              |              +-------------------------+|     Dim_Geography    +--------------+--------------+        Dim_Date         |+----------------------+              |              +-------------------------+|+----------------------+     +--------+-------+      +-------------------------+|   Dim_Demographics   +-----+Fact_Transactions+-----+     Dim_Device_Type     |+----------------------+     +--------+-------+      +-------------------------+|+----------------------+              |              +-------------------------+| Dim_Merchant_Category+--------------+--------------+  Dim_Transaction_Device |+----------------------+              |              +-------------------------+|+-----------+------------+|  Dim_Transaction_Type  |+------------------------+
### Table Definitions
...
To eliminate arbitrary risk thresholds, dynamic economic banding was developed in **Power Query (M-Code)** using percentile-based distribution logic.

### Dynamic Percentile Algorithm (`Amount_Band`)
The engine dynamically parses transaction amounts to calculate the 25th ($Q_1$), 50th ($Q_2$), and 75th ($Q_3$) percentiles:
